...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def get_n_res(
        ...
        # === 修改：检测并使用语义聚类计数 ===
        use_semantic_counts = (
            hasattr(self.reweight.watermark_code_type, "_CLUSTER_COUNTS") 
            and self.reweight.watermark_code_type._CLUSTER_COUNTS is not None
        )

        if use_semantic_counts:
            # 如果加载了语义聚类，必须使用对应的 cluster counts 来重建 splits
            counts = self.reweight.watermark_code_type._CLUSTER_COUNTS.tolist()
            # 简单的校验，防止 n 和预计算的 counts 长度不一致
            if len(counts) == cur_n:
                current_idx = 0
                for count in counts:
                ...
                # 如果 n 不匹配（比如 ablation 实验中测试不同的 n），回退到默认逻辑
                # 但这通常意味着检测会失效，因为 shuffle 是固定的
                 pass 

        # 如果没有语义计数，或者上面的 n 不匹配，则使用默认逻辑
        if not splits: 
            if vocab_size % cur_n == 0:
                splits = (
                    torch.arange(start=0, end=vocab_size)
        ...
        for bsz_idx in range(input_ids.shape[0]):
            ...
            # === 修复：确保 cur_k 是整数 ===
            # cur_k 是 tensor，如果 splits 是 list，直接索引会报错 TypeError
            k_idx = cur_k.item() 
            
            # 兼容 splits 是 Tensor 或 List 的情况
            if isinstance(splits, torch.Tensor):
                current_split = splits[k_idx]
            else:
                current_split = splits[k_idx]

            if current_token[bsz_idx] in watermark_code.shuffle[bsz_idx][current_split]:
                scores.append(1)
            else:
                scores.append(0)
        return scores
