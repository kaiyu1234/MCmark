def get_wps(reweight_type, model_str):
    ...
    if reweight_type == "main_exp":
        ...
        # === 新增代码 ===
    # 加载刚才生成的 .pt 文件
    # 注意：这里需要确保 MCMark_WatermarkCode 已经按照上一步修改好了
        from watermarks.mcmark import MCMark_WatermarkCode 
    
        # 最好把路径做成参数或配置，这里示例写死
        semantic_file = "semantic_shuffle_Mistral_20.pt" 
        print(f"Loading semantic shuffle from {semantic_file}...")
        MCMark_WatermarkCode.load_semantic_shuffle(semantic_file, device="cuda")
...
def transformer_worker(
    ...
    # 新增
    try:
        MCMark_WatermarkCode.load_semantic_shuffle("semantic_shuffle_Mistral_20.pt", device=f"cuda:{gpu_id}")
        print(f"Process {gpu_id}: Loaded semantic shuffle successfully.")
    except Exception as e:
        print(f"Process {gpu_id}: Failed to load semantic shuffle! Error: {e}")
...
def watermark_score_worker(
    ...
    try:
        # 这里路径请根据实际情况修改，确保和生成时一致
        MCMark_WatermarkCode.load_semantic_shuffle("semantic_shuffle_Mistral_20.pt", device=f"cuda:{gpu_id}")
        print(f"Score Worker {gpu_id}: Loaded semantic shuffle.")
    except Exception as e:
        print(f"Score Worker {gpu_id}: Warning - Failed to load semantic shuffle: {e}")
        # 如果是跑基线或者不想用语义水印，这里可以忽略，但如果是测语义水印，这里失败是致命的
